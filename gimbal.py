...
class Gimbal:
    ...
    def __init__(self, addr: str, timeout=30):
        self.addr = addr
        self._parser = DUMLParser()
        self._ble_client = BleakClient(self.addr, services=[self.FFF0], timeout=timeout)
    ...
    async def rotate(self, yaw: int, pitch: int, roll: int, time: int, mode: RotationMode):
        header = bytes([0x55, 0x15, 0x04, 0xa9])
        body = bytearray([0x02, 0x04, 0x01, 0x00, 0x00, 0x04])
    ...
    def _update_battery(self, data: bytes):
...
def _crc16(packet: bytes, init: int) -> int:
    tbl = [
